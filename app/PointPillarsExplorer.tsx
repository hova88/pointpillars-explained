...
export default function PointPillarsExplorer(){
  ...
  return <main>
    ...
    <section className="chapter-shell" id="explorer">
      <aside className={`chapter-nav ${navOpen?"open":""}`}>
      ...
      <div className="chapter-main">
        <div className="chapter-heading">
          <div><span className="chapter-index">{String(chapter.n).padStart(2,"0")} / 25 · {chapter.phase}</span><h2>{chapter.title}</h2><p>{chapter.subtitle}</p></div>
          <a href={sourceUrl(chapter.source)} target="_blank" rel="noreferrer" className={`source-tag ${sourceClass(chapter.source)}`}>{chapter.source} ↗</a>
        </div>
        <div className="symbol-glossary" aria-label="Persistent symbol glossary"><span><b>N</b> points / capacity</span><span><b>P</b> non-empty pillars</span><span><b>D</b> decorated channels</span><span><b>C</b> learned channels</span><span><b>H × W</b> BEV grid</span><span><b>A</b> anchors per cell</span></div>
        <div className="chapter-stage story-stage"><div className="persistent-scene"><PointCloudScene chapter={chapter.n} gridSize={lab.gridSize} range={lab.range}/><div className="story-caption"><span>ONE FRAME · REPRESENTATION {String(chapter.n).padStart(2,"0")}</span><b>{chapter.shape}</b></div></div><LabControls chapter={chapter.n} lab={lab} setLab={setLab}/></div>
        <div className="inspect-row"><div><span>EQUATION</span><code>{chapter.equation}</code></div><div><span>SHAPE LEDGER</span><code>{chapter.shape}</code></div><div><span>OBSERVE</span><p>{chapter.observe}</p></div><div><span>INSPECT</span><p>{chapter.inspect}</p></div></div>
        <Reasoning chapter={chapter}/>
