...
export function scatterPillarFeatures(features, coordinates, height, width) {
  const channels = features[0]?.length ?? 0;
  const image = Array.from({ length: channels }, () =>
    Array.from({ length: height }, () => Array(width).fill(0)),
  );
  features.forEach((feature, pillarListIndex) => {
    const [iy, ix] = coordinates[pillarListIndex] ?? [];
    if (!Number.isInteger(iy) || !Number.isInteger(ix) || iy < 0 || iy >= height || ix < 0 || ix >= width) return;
    feature.forEach((value, channel) => {
      if (channel < channels) image[channel][iy][ix] = value;
    });
  });
  return image;
}
...
export function decodeBox(target, anchor) {
  const diagonal = Math.hypot(anchor.w, anchor.l);
  return {
    x: target[0] * diagonal + anchor.x,
    y: target[1] * diagonal + anchor.y,
    z: target[2] * anchor.h + anchor.z,
    w: Math.exp(target[3]) * anchor.w,
    l: Math.exp(target[4]) * anchor.l,
    h: Math.exp(target[5]) * anchor.h,
    yaw: target[6] + anchor.yaw,
    score: anchor.score,
  };
}
